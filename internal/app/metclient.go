...
func (mc *MetClient) GetMeasurement() ([]WindMeasurement, error) {
	...
	for _, m := range ms.Measurements {
		fmt.Printf("m csv: [%v]\n  isvalid: [%v]\n", m.CSV, m.IsValid)
		if !m.IsValid {
			log.Printf("Measumerement %v is invalid\n", m)
			continue
		}
		...
		if err != nil {
			log.Printf("Unable to convert wind direction %v to int, error: %v\n", vs[1], err)
			continue
		}
		fmt.Printf("direction: %v ˚, speed: %v m/s - %v Knots\n", wd, ws, float64(ws)*1.944)
